...
HT with $\alpha$ $\rightarrow$ $CL = 1 - \alpha$
...
- `infer::hypothesize(null = "point")` and `infer::generate(reps, type = "draw")` or `infer::generate(reps, type = "bootstrap")`
...
```{r ref.label = "viz-ci", echo = FALSE, warning = FALSE, out.width="50%"}
```

]
.panel[.panel-name[Code]

```{r viz-ci, fig.show = "hide"}
boot_means <- ncbirths %>%
...
```{r ref.label = "viz-1", echo = FALSE, warning = FALSE, out.width="50%"}
```

]
.panel[.panel-name[Code]
```{r viz-1, fig.show = "hide", out.height="30%"}
