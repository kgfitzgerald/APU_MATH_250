...
`probationary_introl` | Sometimes the character was given probationary status as an Avenger, this is the date that happened
...
2. *Who are the newest classic Avengers?* Using a single pipeline: Create a new variable called `years_served` that represents the number of years served **as of 2022.**. (Hint: you can use either the `year` variable or `years_since_joining` variables to do this.) Then, arrange the dataset in ascending order of `years_served`. Lastly, select the `name` and `years_served` and display the first three rows.
    - Report the names of the three newest classic Avengers and how long they have served in your narrative text (in addition to the table). While not strictly necessary, you are encouraged to try using inline code to populate the names and years in a sentence instead of typing them manually. 
