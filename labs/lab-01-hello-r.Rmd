...
For this first lab, you will collaborate in your (randomly assigned) groups (see Canvas), but you will each submit your own lab. We want to make sure everyone gets a significant amount of time at the steering wheel in the beginning. In future labs you'll learn about collaborating in RStudio Cloud and produce a single lab report for your team.

# Getting Started - RStudio Cloud

You should have already set up an RStudio Cloud account and been added to the "MATH_250_individual" workspace. If you haven't, touch base with the professor to get set up. To get started on this lab, do the following:

+ Login to your [RStudio Cloud](rstudio.cloud) account
+ Navigate to the "MATH_250_individual" workspace (your workspaces are listed in the left panel of RStudio Cloud)
+ Click on "Continue" next to Week 01 to launch the Week 01 R project. Note, if you did not do AE 01, you may need to click "Start Assignment" instead of "Continue"
+ Open `lab-01.qmd` by clicking on the file name in the Files pane (bottom right quadrant)

The document you are currently reading has all the instructions for Lab 01, but you will complete the lab by adding code and text responses to the `lab-01.qmd` document in RStudio Cloud.
...
In the Quarto (Rmd) file in your project, change the author name to your name, and render the document.
...
🧶 ✅ *This is a good place to pause and Render to make sure there are no errors in your .qmd*
...
Click on the gear icon in on top of the Quarto document, and select "Output Options..." in the dropdown menu.
In the pop up dialogue box go to the Figures tab and change the height and width of the figures, and hit OK when done.
Then, render your document and see how you like the new sizes.
Change and render again and again until you're happy with the figure sizes.
...
You can change them by defining different values directly in your Quarto document as well.

```{r fig-resize-local, fig.margin = FALSE, echo = FALSE, fig.width=3}
...
Once again click on the gear icon in on top of the Quarto document, and select "Output Options..." in the dropdown menu.
In the General tab of the pop up dialogue box try out different Syntax highlighting and theme options.
Hit OK and render your document to see how it looks.
...
🧶 ✅ ⬆️ *Yay, you're done! Render one last time, and verify that everything is displayed correctly in your lab-01.html file. To view the .html, click on the file name in the Files quadrant, and select "View in Web Browser". If everything looks good, you're ready to submit!*

# Submit

To submit, you need to export your .html document from RStudio Cloud and then upload it to the Lab 01 assignment on Canvas. To export the file, select the box next to the appropriate file name in the Files quadrant, click "More" (blue gear icon), and then "Export". 

```{r export, fig.margin = FALSE, echo = FALSE, fig.width=3}
knitr::include_graphics("img/export-from-cloud.png")
```
...
+ Click "Upload File"
+ Click "Choose File"
+ In the pop-up window, navigate to your Downloads folder and select "lab-01.html"
