...
1.  Inspect the `teams` data to determine the `team_id` for the Portland Thorns. Filter the `matches` dataset to only keep matches where Portland was playing. Save this as a new dataset called `portland_matches`. *Confirm that your new data has 196 observations before proceeding.*
...
2.  Include the code above in your lab report. Then, following the above code as a template, create a similar `away_team_info` dataset that renames the columns with the prefix `away_`. Then, join those columns to `portland_matches`. *Confirm `portland_matches` has dimensions 196 x 19 before continuing.*
