...
Next, we will limit our analysis to only the following categories: Physics, Medicine, Chemistry, and Economics. For the remainder of the analyses, we will consider this subset of laureates only. 
...
6.  In a single pipeline, filter for laureates who won their prize in the US, but were born outside of the US, and then create a frequency table (with the `count()` function) for their birth country (`born_country`) and arrange the resulting data frame in descending order of number of observations for each country. Also create a bar plot that displays these frequencies. Which country is the most common?

```{marginfigure}
Hint: using `x = fct_rev(fct_infreq(variable_name))` inside `aes()` will sort the bars from longest to shortest (most frequent to least frequent categories).
```
