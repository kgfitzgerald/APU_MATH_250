...
subtitle: "due Tuesday, October 4 at 11:59p "
output: 
  tufte::tufte_html:
    css: "lab.css"
...
This lab will be completed collaboratively in your randomly assigned Labs 05 - 06 groups (see Canvas). The person randomly assigned to be the Group Leader in Canvas is responsible for submitting the assignment. 

+ In Canvas, navigate to ["RStudio Cloud Links"](https://canvas.apu.edu/courses/42480/pages/rstudio-cloud-links) and click the link to join your group specific `MATH_250 Labs 05-06 G#` workspace. 
...
2. Use the function `mdy_hms()` inside a `mutate()` function to convert the timestamp variable to a date. Then create a histogram of this new variable. What does this reveal about the data?

2. Reproduce the following visualization. Note, you will need to re-order the levels of your year variable.

*Hint: `theme_light()` and the color `navyblue` were used*

![](img/year.png)

3. "On a scale from 1 to 10, how anxious are you about taking MATH 130 - Intro to Statistics?" Create an appropriate visualization or summary table to explore this variable.

4. Reproduce the following visualization. Note, you will need to re-order the levels of your year variable.

*Hint: Set `y = 1` inside your `aes()` function. And add a `theme` layer where `axis.text.y` and `axis.text.y` are set equal to `element_blank()`*

![](img/stats_relevant_career.png)
5. What majors are represented in MATH 130? Create a visualization to explore this variable. Make decisions about how to clean up the levels and combine as necessary. A `case_when()` inside a mutate may be useful. Comment on your decisions. 

6. What is the distribution of gender in MATH 130? Create an appropriate visualization. Make decisions about how to clean up the levels and combine as necessary. A `case_when()` inside a mutate may be useful. Comment on the pros and cons of making this a multiple choice vs open-ended question in a survey. 
...
7. Use a `pivot_longer` to pivot the columns `Mac:PC`. Consider using `names_to = "laptop_type"` and `values_to = "laptop_access"` to name the new pivoted columns. Then, recreate the following visualization. See the animation below to remind you how the `pivot_()` functions work. 
...
8. What is the distribution of race/ethnicity in MATH 130? Create an appropriate visualization to explore this variable. Make decisions about how to clean up the levels and combine as necessary. 
...
1. Create a visualization similar to Exercise 6 to explore what proportion of students in MATH 130 are commuters, athletes, caretakers, etc. 
