...
-   You are ENCOURAGED to use your notes, lecture slides/videos, previous assignments, and the internet
    -   You may utilize already existing resources on the internet, but you may NOT post to any online forums (or ask an AI Chat bot) to ask for help on the exam.\
...
1.  You are interested in creating a data frame with the most "classic" Avengers.
    ...
    *Check: your new data frame should have 27 observations.*

2.  *Who are the newest classic Avengers?* Using a single pipeline: Create a new variable called `years_served` that represents the number of years served **as of 2023.** *(Hint: you can use either the `year` variable or `years_since_joining` variables to do this.)* Arrange the data appropriately to display the newest classic Avengers first.
    Display only necessary columns and the first 10 rows in a nicely formatted table.

    -   Report the names of the three newest classic Avengers and how long they have served in your narrative text (in addition to the table).

3.  *Has the percentage of female Avengers changed over time?* To explore this question, find the percentage of female Avengers in the `classic_avengers` dataset and compare it to the percentage of females among **all** Avengers.
